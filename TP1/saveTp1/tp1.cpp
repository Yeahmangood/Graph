...
void chaineaugmentante(vector < int > & ch, vector< vector< int > > c, vector < vector < int > > f, int s, int t){
	stack <int> P;
	bool visite[n];
	bool stop = false;
		...
		//Si on a pas visité le noeud
			...
			for (int j = 0; j < n; j++){
				...
				if (visite[j] == false){
					...
					if (((c[cur][j] > 0) && (c[cur][j] > f[cur][j])) || ((c[j][cur] > 0) && (f[j][cur] > 0))) {
						P.push(j);
						ch[j] = cur;
					}
				}
...
int increment(vector <int> ch, vector< vector< int > > c, vector< vector < int > > & f, int s, int t){
...
void FlotMax(vector < int > ch, vector < vector < int > > c, vector < vector < int > > f, int s, int t){
	bool stop = false;
	int min;
	while (!stop){
